...
class EventController{
  ...
  static listEventById = (req: Request, res: Response) => {
       const id = req.params.id;
       events.findById(id,(err: mongoose.CallbackError, event: IEvent )=>{
        err ? res.status(400).send({message: `${err.message} - Id não localizado`}) :
          res.status(200).send(event)
       })
  }

  static newEvent = (req: Request, res: Response) =>{  
      let {name, date, start, end, description} = req.body;
      date = new Date(date);
